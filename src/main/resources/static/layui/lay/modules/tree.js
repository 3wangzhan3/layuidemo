                    ...
                    s = o(
                        [
                            ...
                            function () {
                                ...
                                //Bana修改
                                if (i.check && i.check == "checkbox") {
                                    //如果用户选择的风格是checkbox则生成checkbox
                                    var eleStr = "";
                                    if(n.id!=null && n.id!=''){n.id = n.id;}else{n.id = "";}
                                    if(n.bid !=null && n.bid!=''){n.bid = n.bid;}else{n.bid ="";}
                                    if(n.pid!=null && n.pid!=''){n.pid = n.pid;}else{n.pid="";}
                                    if(n.id == null || n.id == ''){
                                        if(n.bid==null || n.bid == ''){
                                            eleStr = i.check && i.check == "checkbox" ? '<input type="checkbox" class="childrenMenu" name="' + i.checkboxName + '" ' + ((n.checked && n.checked == true) ? 'checked="checked"' : "") + (n.checkboxValue ? ('value="' + n.checkboxValue + '"') : "") + 'data-parent-id="' + n.pid + '"'+ (i.checkboxStyle ? (' style="' + i.checkboxStyle + '"') : "") : '';
                                        }else{
                                            eleStr = i.check && i.check == "checkbox" ? '<input type="checkbox" id = "b'+n.bid+'" class="menuBtn" name="' + i.checkboxName + '" ' + ((n.checked && n.checked == true) ? 'checked="checked"' : "") + (n.checkboxValue ? ('value="' + n.checkboxValue + '"') : "") + 'data-parent-id="' + n.pid + '"'+'data-btn-id ="'+n.bid+'"'+ (i.checkboxStyle ? (' style="' + i.checkboxStyle + '"') : "") : '';
                                        }
                                    }else if(n.id != null || n.id!=''){
                                         if(n.bid == null || n.bid == ''){
                                             eleStr = i.check && i.check == "checkbox" ? '<input type="checkbox" class="childrenMenu" name="' + i.checkboxName + '" ' + ((n.checked && n.checked == true) ? 'checked="checked"' : "") + (n.checkboxValue ? ('value="' + n.checkboxValue + '"') : "") + 'data-parent-id="' + n.pid + '"'+ 'id="' + n.id + '"' + (i.checkboxStyle ? (' style="' + i.checkboxStyle + '"') : "") : '';
                                         }else{
                                             eleStr = i.check && i.check == "checkbox" ? '<input type="checkbox" id="b'+n.bid+'" class="menuBtn" name="' + i.checkboxName + '" ' + ((n.checked && n.checked == true) ? 'checked="checked"' : "") + (n.checkboxValue ? ('value="' + n.checkboxValue + '"') : "") + 'data-parent-id="' + n.pid + '"'+'data-btn-id="'+n.bid+'"' + 'id="' + n.id + '"' + (i.checkboxStyle ? (' style="' + i.checkboxStyle + '"') : "") : '';
                                         }
                                    }
                                    if (eleStr.length > 0) {
                                        //给元素添加data-xxx属性
                            ...
                            function () {
                                //Bana修改
                                var titleElemt = '<a href="' + (n.href || "javascript:;") + '" ' + (i.target && n.href ? 'target="' + i.target + '"' : "") +  (n.checkboxValue ? ('value="' + n.checkboxValue + '"') : "") + ">" + ('<i class="layui-icon layui-tree-' + (l ? "branch" : "leaf") + '">' + (l ? n.spread ? t.branch[1] : t.branch[0] : t.leaf) + "</i>")
                                if ((i.check && i.check == "middle-line") && (n.checked && n.checked == true)) {
                                    //如果用户选择的是中间线
                                    titleElemt += ("<cite style='TEXT-DECORATION: line-through'>" + (n.name || "未命名") + "</cite></a>")
                                } else {
                                    titleElemt += ("<cite>" + (n.name || "未命名") + "</cite></a>")
                                }
                                //a链接是否可点击
                                return titleElemt;
                            }(),
